...
use App\Filament\Acc\Resources\AccountResource\Pages\ListAccounts;
use App\Filament\Acc\Resources\AccountResource\Pages\CreateAccount;
use App\Filament\Acc\Resources\AccountResource\Pages\EditAccount;
use App\Enums\AccLevel;
use App\Filament\Acc\Resources\AccountResource\Pages;
use App\Filament\Acc\Resources\AccountResource\RelationManagers;
                ...
                Section::make()
                 ->schema([
                     Radio::make('acc_level')
                         ...
                         ->live()
                         ->inline()
                         ->afterStateHydrated(function (Radio $component,$state,Set $set) {
                         ...
                         ->afterStateUpdated(function (Set $set,$state,Get $get){
                             ...
                             if ($state->value==1) {
                                 ...
                                 $set('num',strval(Account::where('acc_level',1)->max('num')+1));
                             }
                     ...
                     Select::make('theGrand')
                         ...
                         ->required()
                         ->live()
                         ->visible(fn(Get $get,$operation): bool=> ($get('acc_level')->value>1 && $operation=='create'))
                         ->afterStateUpdated(function ($state,Set $set,Get $get): void {
                            $set('grand_id',$state);
                             if ($get('acc_level')->value==2) {
                                 $set('num',Account::where('grand_id',$state)
                                         ->where('acc_level',2)->max('num') + 1);
                                 $set('id',strval($state).'-'.strval($get('num')));
                             }
                             if ($get('acc_level')->value==3) {$set('theFather',null);}
                     ...
                     Select::make('theFather')
                         ...
                         ->required()
                         ->live()
                         ->visible(fn(Get $get,$operation): bool=> ($get('acc_level')->value>2 && $operation=='create'))
                         ->disabled(fn(Get $get): bool=> !$get('theGrand'))
                         ->afterStateUpdated(function ($state,Set $set,Get $get): void {
                             $set('father_id',$state);
                             if ($get('acc_level')->value==3) {
                                 $set('num',Account::where('father_id',$state)
                                         ->where('acc_level',3)->max('num') + 1);
                                 $set('id',strval($state).'-'.strval($get('num')));
                             }
                             if ($get('acc_level')->value==4) {$set('theSon',null);}
                     ...
                     Select::make('theSon')
                         ...
                         ->required()
                         ->live()
                         ->visible(fn(Get $get,$operation): bool=> ($get('acc_level')->value>3 && $operation=='create'))
                         ->disabled(fn(Get $get): bool=> !$get('theFather'))
                         ->afterStateUpdated(function ($state,Set $set,Get $get): void {
                             $set('son_id',$state);
                             if ($get('acc_level')->value==4) {
                     ...
                     TextInput::make('id')
                         ...
                         ->default(function (Get $get){
                             if ($get('acc_level')->value==1) return strval(Account::where('acc_level',1)->max('num')+1);
                         })
                         ->readOnly(),
                     Hidden::make('num')
                         ->default(function (Get $get){
                             if ($get('acc_level')->value==1) return Account::where('acc_level',1)->max('num')+1;
                         }),
